...
assmptns_func <- function(a) {
    ...
    zeta <- zeta %>%
        mutate( sp_vector = sp_vector,  
                ...
                pe_pct   = rep( 0.01, n+1),                         # % of SGA_t+1
                ...
                np_pct   = c(0, rep( 0.05, n)),                     # % of SGA_t+1
...
stmts_cfs <- function(s, a){
    ...
    purchase_MS_net <- s$MS - lag(s$MS, default=0)
    
    CFI <- -a$capX + sp - purchase_MS_net  
    ...
    CFF <- equity_issuance_net + debt_issuance_net + st_debt_borrowing_net - div 
    ...
    stmts_cfs_output <-  list(  a$Year, s$ni, s$bd, chg_DTL_net, -s$gain, -chg_ar, -chg_inv, -chg_pe, chg_ap,
                                chg_wp, chg_itp, OCF, -a$capX , sp, -purchase_MS_net, CFI, equity_issuance_net, debt_issuance_net,
                                st_debt_borrowing_net, -s$div, CFF, chg_in_cash, cash_t_1, cash_t,
                                cash_check)
    
    # A 'negative purchase of M/S' is a 'sale of M/S' from a CF perspective
    
    stmts_cfs__names <-          c("Year", "ni", "bd", "chg_DTL_net", "gain", "chg_ar", "chg_inv", "chg_pe", 
                                   "chg_ap", "chg_wp", "chg_itp", "OCF", "capX" , "sp", "sale_MS_net", "CFI",
                                   "equity_issuance_net", "debt_issuance_net", "st_debt_borrowing_net", "div",
                                    "CFF", "chg_in_cash", "cash_t_1", "cash_t", "cash_check")
...
EVA1 <- function(a, b) {
    ...
    wacc <- c(0, 0.06802, 0.06950, 0.07368,	0.07353, 0.07353) 
